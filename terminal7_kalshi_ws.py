"""Terminal 7 — NBA/NHL Kalshi WebSocket Feed.

Structural clone of terminal6_mlb_kalshi_ws.py with two deltas:

  Δ1 — Subscribes to KXNBAGAME + KXNHLGAME (vs T6's single KXMLBGAME).
       discover_markets() iterates both series_tickers.

  Δ2 — Captures event["series_ticker"] in EVENT_META and writes it into
       each snapshot row. This is what enables T7's Filter 15 (per-series
       exposure cap) without needing an external API.

H-T6-2 ws-refresh fix (session 3) preserved verbatim:
  - _ACTIVE_WS global populated by ws_run_forever, force-closed on subs change
  - _FORCE_RECONNECT flag resets backoff to 1s on operator-triggered close
  - --refresh-test flag for dry-run validation before live redeploy
  - global decls consolidated at top of main() (Lessons #36, #38)
  - Force-reconnect on subs delta within ~30 min refresh tick (vs hours of
    silent invisibility on the prior code path)

H17 Athletics-style normalization is applied at the trader, not here — the
WS just captures everything Kalshi emits.

Auth (mirrors terminal6_mlb_kalshi_ws.py):
...
Usage:
    python3 ~/Documents/terminal7_kalshi_ws.py
    python3 ~/Documents/terminal7_kalshi_ws.py --once  # 30s capture for testing
    python3 ~/Documents/terminal7_kalshi_ws.py --refresh-test  # H-T6-2 dry-run
"""
...
SNAPSHOT_WRITE_SEC = 5
REFRESH_SUBS_SEC = 1800
SCHEMA_VERSION = "v1"

# --- Globals ----------------------------------------------------------
_STOP = False
# H-T6-2 fix: _ACTIVE_WS reference to currently running WSApp; cleared on
# return. Main loop calls _ACTIVE_WS.close() on subs change to force a
# clean reconnect within seconds rather than waiting for a natural close
# (which can take hours on a stable connection — silent kill of new game
# discovery).
_ACTIVE_WS = None
...
def fetch_open_events_for_series(series_ticker: str) -> List[dict]:
    ...
    while True:
        ...
        try:
            ...
            log(f"  [rest] /events failed for {series_ticker}: {e}")
            return out
        if r.status_code != 200:
            log(f"  [rest] /events returned {r.status_code} for {series_ticker}")
            return out
        body = r.json()
...
def fetch_open_events() -> List[dict]:
    """T7: union of open events across NBA + NHL series."""
    out: List[dict] = []
...
def discover_markets() -> List[str]:
    """Return list of market tickers, populating EVENT_META and META as side
    effects. Captures event['series_ticker'] for T7 Filter 15."""
    ...
    for ev in events:
        ...
        EVENT_META[et] = {
            ...
            "series_ticker": ev.get("series_ticker"),   # T7 delta — Filter 15 input
        }
...
def project_row(ticker: str, snap_ts: str) -> Optional[dict]:
    ...
    return {
        ...
        "ticker": ticker,
        "event_ticker": et,
        # T7 delta — series_ticker for Filter 15. Fallback to None if Kalshi
        # didn't return it; trader degrades to per-event cap.
        "series_ticker": ev_meta.get("series_ticker"),
        "title": m.get("title"),
        ...
        "event_title": ev_meta.get("title"),    # required by trader Filter 14
        "_source": "ws",
    }
...
def on_open(ws):
    if not SUBSCRIBED_TICKERS:
        ...
        ws.close()
        return
    log(f"[ws] open; subscribing to {len(SUBSCRIBED_TICKERS)} markets (NBA+NHL)")
    sub = {
        "id": 1,
        "cmd": "subscribe",
        "params": {
...
def main() -> int:
    ...
    ap.add_argument("--refresh-test", action="store_true",
                    help="Validate H-T6-2 fix: subscribe normally, wait 60s, "
                         "drop one ticker, force reconnect, wait 60s for "
                         "resubscribe, exit. Does NOT touch the live ledger.")
    args = ap.parse_args()

    # Globals modified anywhere in main(): consolidate at top per Lessons
    # #36 + #38 — Python disallows mid-function `global` redecl after
    # assignments, hits SyntaxError otherwise.
    ...
    log(f"T7 NBA/NHL Kalshi WS starting; once={args.once} "
        f"series={SERIES_TICKERS}")

    log("[init] discovering markets")
    tickers = discover_markets()
    if not tickers:
        log("[fatal] no markets discovered (NBA + NHL)")
        return 1
    ...
    while not _STOP:
        ...
        if time.time() - last_refresh >= REFRESH_SUBS_SEC:
            ...
            if new_set != old_set:
                ...
                # H-T6-2 fix preserved: force-close active ws so the outer
                # loop in ws_run_forever picks up new SUBSCRIBED_TICKERS via
                # on_open. New playoff games go live within the 30-min
                # refresh tick rather than waiting hours for a natural
                # disconnect. Game 1/2 windows compress this risk further.
                _FORCE_RECONNECT = True
                if _ACTIVE_WS is not None:
                    try:
                        _ACTIVE_WS.close()
                    except Exception as e:
                        log(f"[refresh] ws.close() raised {e} — outer loop will retry")
                else:
                    log("[refresh] no active ws (already reconnecting); "
                        "new tickers picked up on next subscribe")
            last_refresh = time.time()

    log("T7 NBA/NHL Kalshi WS stopped")
    return 0
