"""Terminal 3c — Kalshi Claims Logger.

Snapshots all open KXJOBLESSCLAIMS events at fixed cadence. One JSONL
row per (snapshot, market). Append-only, daily-rotated files.

Phase 1 scope: KXJOBLESSCLAIMS only (Phase 2 adds KXGDP).

Output (per spec §file-layout):
    ~/Documents/terminal3c_data/kalshi_{event_ticker}_{YYYY-MM-DD}.jsonl

Usage:
    python3 ~/Documents/terminal3c_kalshi_logger.py
    python3 ~/Documents/terminal3c_kalshi_logger.py --once
    nohup caffeinate -is python3 ~/Documents/terminal3c_kalshi_logger.py \\
        > /dev/null 2>&1 &
...
OUTPUT_DIR = Path.home() / "Documents" / "terminal3c_data"
...
def snap_once() -> dict:
    ...
    if not events:
        log("  no open KXJOBLESSCLAIMS events returned")
...
def main() -> int:
    ...
    log(f"T3c Kalshi Logger starting. series={SERIES_TICKER} "
